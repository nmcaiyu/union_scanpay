...
int ISO8583Engine_ClearAllFields(ISO8583_Rec *ptIso8583Data)
{
    ...
    for (i = 0; i < ISO8583_MAXFIELD; i++) ptIso8583Data->Field[i].bitf = 0;
...
int ISO8583Engine_SetField(ISO8583_Rec *pIso8583Data, int iFieldNo, unsigned char *pFieldData, int iDataLength)
{
    ...
    if (FldFormatSetFlag != true) return ISOENGINE_NOT_SET_FIELD_FMT;

    iFieldNum = iFieldNo;
    iLength = iDataLength;

    if (iLength <= 0) return ISOENGINE_INVALID_FIELD_LENGTH;
    ...
    if (iLength > ISO8583FldFormat[iFieldNum].iMaxLength) iLength = ISO8583FldFormat[iFieldNum].iMaxLength;
    ...
    if (iLength > 999) return ISOENGINE_TOO_LONG_FILED_LENGTH;
    ...
    if (iLength + pIso8583Data->iOffset >= ISO8583_MAXLENTH) return ISOENGINE_OVER_MAXLENGTH;
...
int ISO8583Engine_GetField(ISO8583_Rec *pIso8583Data, int iFieldNo, unsigned char *pRetFieldData, int iSizeofRetFieldData)
{
    ...
    if (iFieldNum <= 1 || iFieldNum > ISO8583_MAXFIELD) return (-2);
    ...
    if (pIso8583Data->Field[iFieldNum].addr < 0 || pIso8583Data->Field[iFieldNum].addr >= ISO8583_MAXLENTH) return (-4);

    pRpt = &pIso8583Data->cData[pIso8583Data->Field[iFieldNum].addr];
    iLength = pIso8583Data->Field[iFieldNum].len;

    if (iLength < 0 || iLength > 999) return (-3);

    if (iLength > iSizeofRetFieldData) iLength = iSizeofRetFieldData;
...
int ISO8583Engine_HexbufToIso8583(ISO8583_Rec *pIso8583Data, byte *pBuf)
{
    ...
    for (i = 0; i < ISO8583_MAXFIELD; i++) pIso8583Data->Field[i].bitf = 0;
    ...
    for (i = 0; i < iBitnum; i++)
    {
        ...
        {
            if (i == 0 && cBitmask == 0x80) continue;

            if ((pBuf[i + 2] & cBitmask) == 0) continue;
            ...
            if (ISO8583FldFormat[iFieldNum].bType & ISO8583TYPE_VAR)
            {
                ...
                if (iLength > ISO8583FldFormat[iFieldNum].iMaxLength) return (-1);
            }
            else if (ISO8583FldFormat[iFieldNum].bType & ISO8583TYPE_BIN)
                iLength = ISO8583FldFormat[iFieldNum].iMaxLength / 8;
            else
            ...
            if (iLength + iOffSize >= ISO8583_MAXLENTH) return (-3);

            for (k = 0; k < iLength; k++) pIso8583Data->cData[iOffSize++] = *pRpt++;
...
int ISO8583Engine_Iso8583ToHexbuf(ISO8583_Rec *pIso8583Data, byte *pRetBuf, int iSizeRetBuf)
{
    ...
    for (i = 0; i < iBitnum; i++)
    {
        ...
        for (j = 0; j < 8; j++, cBitmask >>= 1)
        {
            ...
            if (pIso8583Data->Field[iFieldNum].bitf == 0) continue;

            if (iFieldNum < 1 || iFieldNum >= ISO8583_MAXFIELD) return (-2);

            if ((cpWpt - pRetBuf) > iSizeRetBuf) return (-3);
            ...
            for (; k < iLength; k++)
            {
                if ((pIso8583Data->Field[iFieldNum].addr + k < 0) || (pIso8583Data->Field[iFieldNum].addr + k >= ISO8583_MAXLENTH)) return (-4);
...
int ISO8583Utils_BCD2LEN(unsigned char *BcdBuf, int *Len, int BcdLen)
{
    ...
    if (BcdLen <= 0 || BcdLen > 10) return -1;
